...
DRESULT USBH_read(BYTE *buff, DWORD sector, UINT count)
{
  ...
  {
    USBH_DbgLog("Aligning read buf %p", buff);
  ...
  else
  {
    ...
    default:
      USBH_ErrLog("USBH_read error %d", info.sense.asc);
      res = RES_ERROR;
      break;
    }
  }
...
DRESULT USBH_write(const BYTE *buff, DWORD sector, UINT count)
{
  ...
  if ((DWORD)buff & 3) /* DMA Alignment issue, do single up to aligned buffer */
  {
    USBH_DbgLog("USBH_write aligning write buf %p", buff);
    while (count--)
    {
  ...
  else
  {
    ...
    default:
      USBH_ErrLog("USBH_write error %d", info.sense.asc);
      res = RES_ERROR;
      break;
    }
  }
