...
/* Read File                                                             */
/*-----------------------------------------------------------------------*/
#if _FS_NO_DIRECT_SECTOR_TRANSFER
static BYTE tempSectorBufferAgainstDirectDma[512];
#endif
...
#if _FS_NO_DIRECT_SECTOR_TRANSFER
                UINT cc_cnt = 0;
                BYTE * tmp_buf = tempSectorBufferAgainstDirectDma;
                do
                {
                    if (disk_read(fp->fs->drv, tmp_buf, sect + cc_cnt, 1) != RES_OK)
                        ABORT(fp->fs, FR_DISK_ERR);
                    memcpy(rbuff + cc_cnt * SS(fp->fs), tmp_buf, SS(fp->fs));
                }
                while(++cc_cnt < cc);
                //ff_memfree(tmp_buf);
#else
...
#if _FS_NO_DIRECT_SECTOR_TRANSFER
                BYTE * tmp_buf = tempSectorBufferAgainstDirectDma;
				if (disk_read(fp->fs->drv, tmp_buf, sect, 1) != RES_OK)	/* Fill sector cache */
					ABORT(fp->fs, FR_DISK_ERR);
                memcpy(fp->buf, tmp_buf, SS(fp->fs));
#else
	...
	for ( ;  btw;							/* Repeat until all data written */
		...
		if ((fp->fptr % SS(fp->fs)) == 0) {	/* On the sector boundary? */
			...
			if (cc) {						/* Write maximum contiguous sectors directly */
                ...
                do
                {
                    memcpy(fp->buf, wbuff + cc_cnt * SS(fp->fs), SS(fp->fs));
                    if (disk_write(fp->fs->drv, fp->buf, sect + cc_cnt, 1) != RES_OK)
                        ABORT(fp->fs, FR_DISK_ERR);
                }
                while(++cc_cnt < cc);
#else
	...
	if (res == FR_OK) {
		...
		if (res == FR_OK) {
			...
			if (!dir) {					/* Root directory */
				...
				ST_WORD(dir + DIR_CrtTime, fno->fctime);
				ST_WORD(dir + DIR_CrtDate, fno->fcdate);
				dj.fs->wflag = 1;
				res = sync_fs(dj.fs);
			}
		}
...
DWORD sect2clust (FATFS* fs, DWORD sect)
{
    if (sect < fs->database) {
        return 0;
    }
...
DWORD
find_sclust (FATFS* fs, DWORD clst)
{
	DWORD s, val;

    if (clst == 0) {
        return 0;
    } else if (clst < 2 || clst >= fs->n_fatent) {	/* Check range */
		return 1;	/* Internal error */
	} else if (val = get_fat(fs, clst), (val == 0x0FFF) || (val == 0x0FFFFFFF)) {
        return clst;    /* Already at start cluster */
    } else {
		s = 2;
///printf("clst %lu start at ", clst);
		while (val = get_fat(fs, s), (val >= 2) && (val != UINT32_MAX))
        {
            // TODO improve performance by seeking backward from initial cluster?
            if (val == clst) { /* This entry is referencing the cluster that we're coming from */
                clst = s;
                s = 1;  /* restart search with new cluster */
            }
            s++;
        }
///printf("%lu\n", clst);
...
void testclusters(FATFS* fs)
{
	DWORD s;
    for(s = 0; s < 20; s++)
    {
        printf("get_fat(%lu) = %lu\n", s, get_fat(fs, s));
    }
}
